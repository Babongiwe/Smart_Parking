...
export const Sidebar: React.FC = () => {
    ...
    switch (currentRole) {
      ...
      default:
        return [
          ...
          { id: 'visitor-register', label: 'Visitor Guest Pass', icon: Users },
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 h-[calc(100vh-85px)] sticky top-[85px] shadow-sm">
      ...
      {/* Footer with Sign Out Button & System Status */}
      <div className="p-4 border-t border-slate-200 bg-slate-50/80 space-y-3">
        <button
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 text-xs font-bold transition-colors"
        >
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
          <div className="flex items-center gap-1.5 font-medium">
            ...
            <span>ALPR Online</span>
          </div>
          <span className="text-[10px] text-slate-400">Bloemfontein 2026</span>
        </div>
      </div>
    </aside>
  );
